...
def get_population_initialization(data: Dict[str, Any]) -> Callable:
    ...
    if "population_initialization" not in data["operators"]:
        raise KeyError(
            "В исходных данных отсутствует информация по операторам ГА (operators/population_initialization)"
        )
    methods = {
    ...
    if data["operators"]["population_initialization"] in methods.keys():
        ...
        raise KeyError(
            "В исходных данных отсутствует информация по операторам ГА (operators/population_initialization/method)"
        )
